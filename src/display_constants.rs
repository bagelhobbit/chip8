pub const HEIGHT: u16 = 32;

pub const WIDTH: u16 = 64;

// ****
// *  *
// *  *
// *  *
// ****
/// Sprite data for 0
pub const ZERO: [u8; 5] = [0xF0, 0x90, 0x90, 0x90, 0xF0];

//  * 
// ** 
//  * 
//  * 
// ***
/// Sprite data for 1
pub const ONE: [u8; 5] = [0x20, 0x60, 0x20, 0x20, 0x70];

// ****
//    *
// ****
// *   
// ****
/// Sprite data for 2
pub const TWO: [u8; 5] = [0xF0, 0x10, 0xF0, 0x80, 0xF0];

// ****
//    *
// ****
//    *
// ****
/// Sprite data for 3
pub const THREE: [u8; 5] = [0xF0, 0x10, 0xF0, 0x10, 0xF0];

// *  *
// *  *
// ****
//    *
//    *
/// Sprite data for 4
pub const FOUR: [u8; 5] = [0x90, 0x90, 0xF0, 0x10, 0x10];

// ****
// *   
// ****
//    *
// ****
/// Sprite data for 5
pub const FIVE: [u8; 5] = [0xF0, 0x80, 0xF0, 0x10, 0xF0];

// ****
// *   
// ****
// *  *
// ****
/// Sprite data for 6
pub const SIX: [u8; 5] = [0xF0, 0x80, 0xF0, 0x90, 0xF0];

// ****
//    *
//   * 
//  *  
//  *  
/// Sprite data for 7
pub const SEVEN: [u8; 5] = [0xF0, 0x10, 0x20, 0x40, 0x40];

// ****
// *  *
// ****
// *  *
// ****
/// Sprite data for 8
pub const EIGHT: [u8; 5] = [0xF0, 0x90, 0xF0, 0x90, 0xF0];

// ****
// *  *
// ****
//    *
// ****
/// Sprite data for 9
pub const NINE: [u8; 5] = [0xF0, 0x90, 0xF0, 0x10, 0xF0];

// ****
// *  *
// ****
// *  *
// *  *
/// Sprite data for A
pub const A: [u8; 5] = [0xF0, 0x90, 0xF0, 0x90, 0x90];

// *** 
// *  *
// *** 
// *  *
// *** 
/// Sprite data for B
pub const B: [u8; 5] = [0xE0, 0x90, 0xE0, 0x90, 0xE0];

// ****
// *   
// *   
// *   
// ****
/// Sprite data for C
pub const C: [u8; 5] = [0xF0, 0x80, 0x80, 0x80, 0xF0];

// *** 
// *  *
// *  *
// *  *
// *** 
/// Sprite data for D
pub const D: [u8; 5] = [0xE0, 0x90, 0x90, 0x90, 0xE0];

// ****
// *   
// ****
// *   
// ****
/// Sprite data for E
pub const E: [u8; 5] = [0xF0, 0x80, 0xF0, 0x80, 0xF0];

// ****
// *   
// ****
// *   
// *   
/// Sprite data for F
pub const F: [u8; 5] = [0xF0, 0x80, 0xF0, 0x80, 0x80];
